...
public class MonteCarloAlgorithm implements Algorithm {
    ...
    public Move getMove(Board board, ComputerPlayerHardnessLevel hardnessLevel, int playerId, int amountOfWallsLeft) {
        ...
        for (Move move : moves) {
            ...
            for (int i = 0; i < simulations; ++i) {
                ...
                if (simulateGame(copy, 3 - playerId, amountOfWallsLeft, amountOfWallsLeft) == playerId) {
                    ++wins;
                }
            }
    ...
    private int simulateGame(Board board, int currentPlayer, int walls1, int walls2) {
    ...
    private boolean isWin(Board board, int playerId, int size) {
        ...
        return (playerId == 1 && row == size - 1) || (playerId == 2 && row == 0);
    }

    private int evaluateWinner(Board board, int size) {
        int d1 = calculateShortestPath(board, getCurrentPosition(board, 1), size - 1);
        int d2 = calculateShortestPath(board, getCurrentPosition(board, 2), 0);
        return d1 < d2 ? 1 : 2;
    }
}